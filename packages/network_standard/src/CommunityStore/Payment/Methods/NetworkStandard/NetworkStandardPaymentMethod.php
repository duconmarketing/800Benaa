...
class NetworkStandardPaymentMethod extends StorePaymentMethod 
{
    ...
    public function redirectForm()
    {
        ...
        // $outletRef = '0c0f97e2-748a-492c-9ef1-f658257010aa';
        // $apikey = 'NzZlNWM2YWItNmY2My00YjVhLTgyZGQtZmIwNGQzNTNhNzRjOmU0MThiZmE4LTRiMDEtNGM0OC04ODY2LWVkYTI5YjJlMmI0Nw==';
        ...
        $tokenResponse = $this->curlRequest("POST", $idServiceURL, $tokenHeaders, "{\"realmName\":\"NetworkInternational\"}");
...
//        $order->language = "en";                                        // Payment page language ('en' or 'ar' only)
        $order->merchantOrderReference = 'BNA'. $storeOrder->getOrderID();
        $order->merchantAttributes = new \stdClass();
        $order->merchantAttributes->redirectUrl = 'https://www.800benaa.com/checkout/networkresponse';
...
//        $order->merchantAttributes->skipConfirmationPage = 'TRUE';
    ...
    public static function decryptData()
    {
        ...
        // $idServiceURL = "https://api-gateway.ngenius-payments.com/identity/auth/access-token";
        // $txnServiceURL = "https://api-gateway.ngenius-payments.com/transactions/outlets/$outletRef/orders/" . $raw_get_data['ref'];
        //------------------------------------------

        $tokenHeaders = array("accept: application/vnd.ni-identity.v1+json", "authorization: Basic " . $apikey, "content-type: application/vnd.ni-identity.v1+json");
        $tokenResponse = self::curlRequest("POST", $idServiceURL, $tokenHeaders, "{\"realmName\":\"NetworkInternational\"}");
        $tokenResponse = json_decode($tokenResponse);
        $access_token = $tokenResponse->access_token;
        ...
        if ($orderStatus == 'CAPTURED') {
            ...
            $order = StoreOrder::getByID(substr($orderNumber,3));
            ...
            $redirect->setTargetUrl('/checkout/fail?error=Payment Failed');
            $redirect->send();
        }
    }
